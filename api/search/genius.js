...
const extractLyrics = (html) => {
  ...
  for (const c of ld) {
    try {
      ...
      const ly =
        typeof obj.lyrics === "string" ? obj.lyrics : obj.lyrics?.text;
      if (ly) {
  ...
  if (pre) {
    try {
      ...
      const m = str.match(/"lyrics"\s*:\s*"([^"]{50,})"/);
      if (m) {
        const t = m[1].replace(/\\n/g, "\n").replace(/\\"/g, '"');
        if (t.length > 50) return t;
      }
  ...
  if (m) {
    ...
    if (t.length > 30 && !/translation|read more|lyrics/i.test(t)) return t;
  }
...
const ddgExtract = async (q, h) => {
  ...
  const r = await axios.get(`https://duckduckgo.com/html/?q=${qq}`, {
    headers: h,
  });
  const $ = cheerio.load(r.data);
  let url = null,
...
const searchLyrics = async (q = "") => {
  ...
  try {
    ...
    if (e?.response?.status === 403) {
      ...
      if (!link && !snippet) throw new Error("Fallback gagal");
...
module.exports = {
  name: "GeniusLyrics",
  desc: "Search & scrape lyrics from Genius (with DuckDuckGo fallback for CF).",
  category: "Scraper",
  params: ["query"],
